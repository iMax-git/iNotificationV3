...
window.addEventListener("message", function(event) {
    ...
    switch (action) {
        case "Notification":
            ...
            if (!exist) {
                Notification(data.title, data.subtitle, data.icon, data.body, data.duration, data.location, data.isMiniMapHidden);
            } else {
        ...
        case "HelpNotification":
            ...
            if (!exist) {
                HelpNotification(data.title, data.icon, data.body, data.duration, data.location, data.isMiniMapHidden);
            } else {
                this.clearTimeout(exist.dataset.to1);
                this.clearTimeout(exist.dataset.to2);
                this.clearTimeout(exist.dataset.to3);
